...
func _test_shipped_catalog(t: TestCtx) -> void:
	...
	t.near(s.value_mult(&"bumpers"), pow(1.25, 3.0) * 1.05, 1e-9, "Can Deposits x3 under Brass Balls")
	t.near(s.value_mult(&"spinner"), pow(1.3, 2.0) * 1.05, 1e-9, "Fast Fingers x2 under Brass Balls")
	...
	_money(t, s.launder_cap(), "18K", "$2K plus $8K per washer level")
	...
	t.near(s.collect_minutes(), 5.0 * pow(1.35, 2.0), 1e-9, "Muscle on the Block x2")
