...
extends Node3D
## LUCKY'S TOWER (docs/19 §3.3), the signature mechanism. A ball into Lucky's scoop is taken by
## the drum and tumbles behind the porthole for the length of the wash cycle, then the glass
## service lift carries it up and lets it out through the side door into the Alley. With the
## Penthouse owned and the Sit-Down lit, the lift keeps going to the top floor, seats the ball at
## the Commission table for a beat, and brings it back down.
##
## The ball is out of physics while it rides (BallHold) and is handed back at the side door with
...
const ALLEY_DOOR_H := 0.45
const TOP_FLOOR_H := 1.20
const SIT_SECONDS := 2.4
const DOOR_OUT := Vector3(-7.5, 0.0, -0.6)    ## side-door throw into the nest (table space)
const CHUTE_SECONDS := 0.16
...
var _going_up_to: float = ALLEY_DOOR_H
...
func drum_center() -> Vector3:
	...
	return Vector3(r.position.x + r.size.x * 0.58, 0.30, r.position.y + r.size.y - 0.02)


## The lift runs up the tower's Alley side, so its door opens straight into the nest.
func lift_bottom() -> Vector3:
	var r := Layout.TOWER_RECT
	return Vector3(r.position.x + 0.11, 0.20, r.position.y + r.size.y - 0.16)


func door_point() -> Vector3:
	return Vector3(Layout.MIRROR_X + Layout.NEST_HALF - 0.17, Feel.BALL_RADIUS + 0.02, -3.40)
...
func _build_body() -> void:
	...
	walls.chain(PackedVector2Array([Vector2(x0, z1), Vector2(x1, z1)]), 0.06)
	walls.bar(Vector2(x0, z0), Vector2(x0, z1), 0.06)
...
func _build_scoop() -> void:
	...
	# the back wall of the scoop: a ball that runs up the lane too hot stops here
	var back := WallBuilder.make_body("ScoopBack")
	add_child(back)
	var w := WallBuilder.new(back, 0.3)
	var s := Layout.SCOOP_AT
	w.arc(s, SCOOP_R + 0.05, 200.0, 340.0, 10, 0.05)
	w.build_mesh(_lib.brass_dark(), _lib.brass())
...
func _build_look() -> void:
	...
	var w := r.size.x
	var d := r.size.y
	# ground floor: LUCKY'S laundromat in brick with the drum window
	var st := MeshLib.begin()
	MeshLib.box(st, Vector3(x0 + w * 0.5, 0.30, z0 + d * 0.5), Vector3(w, 0.60, d), 0.6)
...
func _on_scoop(body: Node3D) -> void:
	...
	if _phase != Phase.IDLE:
		# the lift is busy with another guy: the scoop spits this one back down the lane
		b.set_velocity(Vector3(-0.25, 0.0, 1.0).normalized() * 9.0)
		return
	_ball = b
	BallHold.take(b)
	_phase = Phase.DRUM
	...
	match _phase:
		...
		Phase.TO_LIFT:
			...
			if _ball.table_position().distance_to(lb) < 0.03 or _t > 0.6:
				_phase = Phase.UP
				_t = 0.0
				_going_up_to = TOP_FLOOR_H if (penthouse_open and sitdown_lit) else ALLEY_DOOR_H
				AudioDirector.play(&"chime_a")
		Phase.UP:
			var p := _ball.table_position()
			var to := Vector3(lift_bottom().x, _going_up_to, lift_bottom().z)
			...
			if next.distance_to(to) < 0.01:
				_t = 0.0
				if is_equal_approx(_going_up_to, TOP_FLOOR_H):
					_phase = Phase.SEATED
					sitdown_lit = false
		...
		Phase.DOWN:
			...
			var to2 := Vector3(lift_bottom().x, ALLEY_DOOR_H, lift_bottom().z)
		...
		Phase.OUT:
			if _t < CHUTE_SECONDS:
				var from := Vector3(lift_bottom().x, ALLEY_DOOR_H, lift_bottom().z)
				BallHold.steer(_ball, from.lerp(door_point(), _t / CHUTE_SECONDS), delta)
				return
			var b := _ball
			_phase = Phase.IDLE
			_ball = null
			_cool = 0.4
			BallHold.release(b, door_point(), DOOR_OUT)
...
func set_hardware_active(active: bool) -> void:
	...
	if not active and _ball != null and is_instance_valid(_ball) and _phase != Phase.IDLE:
		BallHold.release(_ball, door_point(), DOOR_OUT)
		_phase = Phase.IDLE
		_ball = null
	if _scoop != null:
